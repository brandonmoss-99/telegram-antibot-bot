	...
	def __init__(self, message, configHandler, uData):
		...
		if self.uData.userExists(userKey):
			if ((self.uData.getData(userKey, 'timeSentFirstMessage') == None) or 
				(int(time.time()) - self.uData.getData(userKey, 'timeSentFirstMessage') <= self.configHandler.getCustomGroupConfig(self.chat['id'])['timeToRestrictForwards'])):
